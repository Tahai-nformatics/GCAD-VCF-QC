...
#'obs_homo2': {((1, 1), (1, 1)): 0, ((2, 2), (2, 2)): 0, ((3, 3), (3, 3)): 0}}    
    ...
    #        mi.sa.sa_collection[k].tallySA['missing'] += 1                                 # Add Missing samples to SA
            sm['GT'] = (None, None)
            tallyMissingSample_multiallelic(k, sm)
            continue
        try:
            if (sm['DP'] < cfg.MINDP or sm['GQ'] < cfg.MINGQ):
                if sm['GT'][0] == sm['GT'][1]: #Homozygous Sample (REF or ALT)  
                    if sm['GT'] == (0,0):
                        ...
                        mi.sa.sa_collection_multiallelic[k].tallySA['failing_obs_homo1'] += 1    # Add Failing samples to SA
                    else: #Homozygous ALT
                ...
                sm['GT'] = (None, None)
                gt_failed += 1
                tallyFailedSample_multiallelic(k, sm)
                depth_sum += sm['DP']
                continue
        except TypeError:
            for classification in failing_d.keys():
                    for key, values in failing_d[classification].items():
                        if sm['GT'] in key:
                            ...
                            mi.sa.sa_collection_multiallelic[k].tallySA['failing_'+ classification] += 1
            sm['GT'] == (None, None)
            tallyFailedSample_multiallelic(k, sm)
            gt_failed += 1
            continue
...
#def tallyMissingSample_multiallelic(k, sm):
#    mi.sa.tally_multiallelic(k, sm, 0)

def tallyFailedSample(k, sm):
    mi.sa.tally(k, sm, 1)

#def tallyFailedSample_multiallelic(k, sm):
#    mi.sa.tally_multiallelic(k, sm, 1)


def tallyPassingSample(k,sm):
    #print('tallying passing sample')
    mi.sa.tally(k, sm, -1)

#def tallyPassingSample_multiallelic(k,sm):
    #print('tallying passing sample')
#    mi.sa.tally_multiallelic(k, sm, -1)

def tallyFailedSample_multiallelic(k, sm):
    mi.sa.sa_collection_multiallelic[k].tallySA[ -9 ] += 1
    if sm['DP'] != None:
        mi.sa.sa_collection_multiallelic[k].dp_total += sm['DP']
    return

def tallyPassingSample_multiallelic(k, sm):
    mi.sa.sa_collection_multiallelic[k].dp_total += sm['DP']
    # Additional tallies for passing samples
    return

def tallyMissingSample_multiallelic(k, sm):
    mi.sa.sa_collection_multiallelic[k].tallySA[(None, None)] += 1
    return
